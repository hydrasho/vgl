...
public class Event {
	private Event () {
		s_builder = new StringBuilder.sized(255);
	}
	...
	public void poll() {
		...
		switch ((int)event.type) {
			...
			case SDL.EventType.TEXTINPUT:
				s_builder.append(event.text.text ?? "");
				onTextInput(s_builder.str);
				break;
		}
	}

	public void start_text_input() {
		SDL.Input.TextInput.start();
		s_builder.erase(0, s_builder.len);
	}
	
	public void stop_text_input() {
		SDL.Input.TextInput.stop();
	}

	public signal void onTextInput(string text) {
